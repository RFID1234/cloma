...
+function(n) {
    ...
    var t = function(t, i) {
        ...
        this.options.remote && this.$element.find(".modal-content").on("load", this.options.remote, n.proxy(function() {
...
!function(n) {
    ...
    return n.extend(n.fn, {
        ...
        rules: function(t, i) {
            var e, s, f, u, o, h, r = this[0], c = "undefined" != typeof this.attr("contenteditable") && "false" !== this.attr("contenteditable");
            if (null != r && (!r.form && c && (r.form = this.closest("form")[0],
    ...
    n.extend(n.validator, {
        ...
        prototype: {
            init: function() {
                ...
                this.invalid = {};
                this.reset();
                var t, r = this.currentForm, u = this.groups = {};
            ...
            elements: function() {
                ...
                return n(this.currentForm).find("input, select, textarea, [contenteditable]").not(":submit, :reset, :image, :disabled").not(this.settings.ignore).filter(function() {
                    ...
                    this.name = r),
                    this.form === t.currentForm && !(r in i || !t.objectLength(n(this).rules())) && (i[r] = !0,
                    !0)
                })
            },
            clean: function(t) {
            ...
            elementValue: function(t) {
                var i, r, u = n(t), f = t.type, e = "undefined" != typeof u.attr("contenteditable") && "false" !== u.attr("contenteditable");
                return "radio" === f || "checkbox" === f ? this.findByName(t.name).filter(":checked").val() : "number" === f && "undefined" != typeof t.validity ? t.validity.badInput ? "NaN" : u.val() : (i = e ? u.text() : u.val(),
                "file" === f ? "C:\\fakepath\\" === i.substr(0, 12) ? i.substr(12) : (r = i.lastIndexOf("/"),
            ...
            check: function(t) {
                ...
                var u, f, r, e, i = n(t).rules(), c = n.map(i, function(n, t) {
                    return t
                }).length, s = !1, h = this.elementValue(t);
        ...
        normalizeRules: function(t, i) {
            ...
            n.each(["rangelength", "range"], function() {
                var n;
                t[this] && (Array.isArray(t[this]) ? t[this] = [Number(t[this][0]), Number(t[this][1])] : "string" == typeof t[this] && (n = t[this].replace(/[\[\]]/g, "").split(/[\s,]+/),
...
(function(n, t, i) {
    n.fn.backstretch = function(r, u) {
        return (r === i || 0 === r.length) && n.error("No images were supplied for Backstretch"),
        0 === n(t).scrollTop() && t.scrollTo(0, 0),
        this.each(function() {
            var i = n(this)
              , t = i.data("backstretch");
            if (t) {
                if ("string" == typeof r && "function" == typeof t[r]) {
                    t[r](u);
                    return
                }
                u = n.extend(t.options, u);
                t.destroy(!0)
            }
            t = new c(this,r,u);
            i.data("backstretch", t)
        })
    }
    ;
    n.backstretch = function(t, i) {
        return n("body").backstretch(t, i).data("backstretch")
    }
    ;
    n.expr[":"].backstretch = function(t) {
        return n(t).data("backstretch") !== i
    }
    ;
    n.fn.backstretch.defaults = {
        centeredX: !0,
        centeredY: !0,
        duration: 5e3,
        fade: 0
    ...
    var l = {
      ...
      , c = function(i, r, u) {
        this.options = n.extend({}, n.fn.backstretch.defaults, u || {});
        this.images = n.isArray(r) ? r : [r];
        n.each(this.images, function() {
            n("<img />")[0].src = this
        });
        this.isBody = i === document.body;
        this.$container = n(i);
        this.$root = this.isBody ? o ? n(t) : n(document) : this.$container;
        i = this.$container.children(".backstretch").first();
        this.$wrap = i.length ? i : n('<div class="backstretch"><\/div>').css(l).appendTo(this.$container);
        ...
        n(t).on("resize.backstretch", n.proxy(this.resize, this)).on("orientationchange.backstretch", n.proxy(function() {
    ...
    c.prototype = {
        resize: function() {
            try {
                var f = {
                    left: 0,
                    top: 0
                }, e = this.isBody ? this.$root.width() : this.$root.innerWidth(), i = e, r = this.isBody ? t.innerHeight ? t.innerHeight : this.$root.height() : this.$root.innerHeight(), n = i / this.$img.data("ratio"), u;
                n >= r ? (u = (n - r) / 2,
                this.options.centeredY && (f.top = "-" + u + "px")) : (n = r,
                i = n * this.$img.data("ratio"),
                u = (i - e) / 2,
                this.options.centeredX && (f.left = "-" + u + "px"));
                this.$wrap.css({
                    width: e,
                    height: r
        ...
        show: function(t) {
            if (!(Math.abs(t) > this.images.length - 1)) {
                var i = this
                  , u = i.$wrap.find("img").addClass("deleteable")
                  , r = {
                    relatedTarget: i.$container[0]
                };
                return i.$container.trigger(n.Event("backstretch.before", r), [i, t]),
                this.index = t,
                clearInterval(i.interval),
                i.$img = n("<img />").css(a).bind("load", function(f) {
                    var e = this.width || n(f.target).width();
                    f = this.height || n(f.target).height();
                    n(this).data("ratio", e / f);
                    n(this).fadeIn(i.options.speed || i.options.fade, function() {
                        u.remove();
                        i.paused || i.cycle();
                        n(["after", "show"]).each(function() {
                            i.$container.trigger(n.Event("backstretch." + this, r), [i, t])
                        })
                    });
                    i.resize()
                }).appendTo(i.$wrap),
        ...
        cycle: function() {
            ...
            this.interval = setInterval(n.proxy(function() {
                this.paused || this.next()
            }, this), this.options.duration)),
            this
        },
        destroy: function(i) {
            n(t).off("resize.backstretch orientationchange.backstretch");
            clearInterval(this.interval);
            i || this.$wrap.remove();
            this.$container.removeData("backstretch")
        }
    };
    var o, u = navigator.userAgent, s = navigator.platform, r = u.match(/AppleWebKit\/([0-9]+)/), r = !!r && r[1], f = u.match(/Fennec\/([0-9]+)/), f = !!f && f[1], h = u.match(/Opera Mobi\/([0-9]+)/), v = !!h && h[1], e = u.match(/MSIE ([0-9]+)/), e = !!e && e[1];
    o = !((-1 < s.indexOf("iPhone") || -1 < s.indexOf("iPad") || -1 < s.indexOf("iPod")) && r && 534 > r || t.operamini && "[object OperaMini]" === {}.toString.call(t.operamini) || h && 7458 > v || -1 < u.indexOf("Android") && r && 533 > r || f && 6 > f || "palmGetResource"in t && r && 534 > r || -1 < u.indexOf("MeeGo") && -1 < u.indexOf("NokiaBrowser/8.5.0") || e && 6 >= e)
}
)(jQuery, window);
/*!
Waypoints - 4.0.1
...
function() {
    ...
    function n(u) {
        ...
        r += 1;
        t.windowContext || (t.windowContext = !0,
        t.windowContext = new n(window));
    ...
    n.prototype.checkEmpty = function() {
        ...
        n && t && !r && (this.adapter.off(".waypoints"),
        delete i[this.key])
    }
    ;
    ...
    n.prototype.handleScroll = function() {
        ...
        for (r in i) {
            ...
            for (f in this.waypoints[r])
                if (n = this.waypoints[r][f],
                null !== n.triggerPoint) {
                    var e = t.oldScroll < n.triggerPoint
                      , o = t.newScroll >= n.triggerPoint
                      , l = e && o
                      , a = !e && !o;
                    (l || a) && (n.queueTrigger(c),
                    u[n.group.id] = n.group)
                }
        }
        for (s in u)
            u[s].flushTriggers();
...
$(function() {
    var u = !1, p = $(".navbar"), s = $(".navbar-collapse"), w = $(".navbar-nav > li > a"), b = $window.scrollTop(), n, h, t, i, f, c, e, o, r, l;
    $('[href*="#section_"]').on("click", function(n) {
        if (u)
            n.preventDefault();
        else if (u = !0,
        s.collapse("hide"),
    ...
    n = $(".banner__carousel__img");
    n.length && (h = n.data("images").split(","),
    n.backstretch(h, {
        duration: 5e3,
        fade: 750
    }),
    ...
    t.on("slid.bs.carousel", function(t) {
        var i = $(t.relatedTarget).index();
        n.backstretch("show", i)
    });
    if (t.waypoint(function(n) {
        n == "down" ? t.carousel("pause") : t.carousel("cycle")
    }, {
        offset: function() {
    ...
    i.length) {
        ...
        $window.scroll(function() {
            i.hasClass("parallax") && setTimeout(function() {
    ...
    $("#modal_portfolio").on("show.bs.modal", function(n) {
        var t = $(n.relatedTarget)
          , i = $(this)
          , r = t.data("heading")
          , u = t.data("img")
          , f = t.data("content");
        i.find("#modal_portfolio__heading").text(r);
        i.find("#modal_portfolio__img").attr("src", u);
        i.find("#modal_portfolio__content").text(f)
    });
    f = $(".stats__item__value");
    f.length && f.each(function() {
        var n = $(this);
        n.waypoint(function() {
            n.not(".finished").countTo({
                onComplete: function() {
                    n.addClass("finished")
                }
            })
    ...
    e = $(".screenshots__carousel");
    e.length && e.owlCarousel({
        items: 3,
        loop: !0
    });
    o = $("#fullpage");
    o.length && (r = $("#fullpage__carousel"),
    l = r.data("images").split(","),
    r.backstretch(l, {
        duration: 5e3,
        fade: 750
    }),
    ...
    o.fullpage({
        ...
        onLeave: function(n, t) {
            r.backstretch("show", t - 1);
            s.collapse("hide")
        },
        afterLoad: function() {
            $("section.active").is(".section_stats") && $(".stats__item__value:not(.finished)").countTo({
                onComplete: function() {
